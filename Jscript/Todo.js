...
function editTodo(index) {
   const newTodo = prompt('Редактировать задачу:', todos[index]);
    if (newTodo !== null) {
...
// Добавляем кнопку редактирования в каждый элемент списка
function renderTodos(todos) {
 todoList.innerHTML = ''; // Очищаем список перед рендером
    todos.forEach((todo, index) => {
    const li = document.createElement('li');
    li.textContent = todo;
    li.innerHTML += `
        <button onclick="editTodo(${index})">Редактировать</button>
...
// Добавляем кнопку выполнения в каждый элемент списка
function renderTodos(todos) {
 todoList.innerHTML = ''; // Очищаем список перед рендером
    todos.forEach((todo, index) => {
    const li = document.createElement('li');
